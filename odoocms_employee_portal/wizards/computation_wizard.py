...
class TaxCertificateWizard(models.Model):
    ...
    #     for line in self:
    #         if line.company_id:
    #             line.company_number = line.company_id
    ...
    def onchange_company_number(self):
        for line in self:
            if line.company_number:
                line.company_id = self.env['res.company'].search([], limit=1).id
                # ('segment1', '=', line.company_number)

    def check_report(self):
        data = {}
        data['form'] = \
        self.read(['company_id', 'date_from', 'date_to', 'employee_id', 'certificate_line_ids', 'bank', 'branch'])[0]
        return self._print_report(data)

    def _print_report(self, data):
        data['form'].update(
            self.read(['company_id', 'date_from', 'date_to', 'employee_id', 'certificate_line_ids', 'bank', 'branch'])[
                0])
        return self.env.ref('odoocms_employee_portal.employee_action_report').report_action(self, data=data,
                                                                                                config=False)
        ...
        for line in self:
            if line.date_from and line.date_to and line.employee_id and line.branch and line.bank:
                payslips = self.env['hr.payslip'].search(
                    [('employee_id', '=', line.employee_id.id), ('date_to', '>=', line.date_from),
